...
#define USE_LOGGING IN_USE

#if !USING( USE_LOGGING )

#define LOG( ... ) do {} while(0)
#define LOG_WARN( ... ) do {} while(0)
#define LOG_ERR( ... ) do {} while(0)

#else // #if USING( USE_LOGGING )
...
consteval LengthSpecifier GetLength( std::string_view fmt, size_t pos )
{
    ...
    {
    case 'h':
        if ( pos < fmt.size() && fmt[pos+1] == 'h' ) return LengthSpecifier::CHAR;
        return LengthSpecifier::SHORT;
    case 'l':
        if ( pos < fmt.size() && fmt[pos+1] == 'l' ) return LengthSpecifier::LONG_LONG;
        return LengthSpecifier::LONG;
    case 'j':
        return LengthSpecifier::INTMAX;
    case 'z':
        return LengthSpecifier::SIZE_T;
    case 't':
        return LengthSpecifier::PTRDIFF;
    case 'L':
        return LengthSpecifier::LONG_DOUBLE;
    }
...
consteval FormatCheckError CheckIntegralLength( LengthSpecifier fmtLen, size_t typeLen )
{
    ...
    case LengthSpecifier::CHAR:
        if ( typeLen != sizeof( char ) ) return FormatCheckError::EXPECTED_CHAR_LENGTH;
        break;
    case LengthSpecifier::SHORT:
        if ( typeLen != sizeof( short ) ) return FormatCheckError::EXPECTED_SHORT_LENGTH;
        break;
    case LengthSpecifier::LONG:
        if ( typeLen != sizeof( long ) ) return FormatCheckError::EXPECTED_LONG_LENGTH;
        break;
    case LengthSpecifier::LONG_LONG:
        if ( typeLen != sizeof( long long ) ) return FormatCheckError::EXPECTED_LONG_LONG_LENGTH;
        break;
    case LengthSpecifier::INTMAX:
        if ( typeLen != sizeof( intmax_t ) ) return FormatCheckError::EXPECTED_INTMAX_LENGTH;
        break;
    case LengthSpecifier::SIZE_T:
        if ( typeLen != sizeof( size_t ) ) return FormatCheckError::EXPECTED_SIZE_T_LENGTH;
        break;
    case LengthSpecifier::PTRDIFF:
        if ( typeLen != sizeof( std::ptrdiff_t ) ) return FormatCheckError::EXPECTED_PTRDIFF_LENGTH;
        break;
    case LengthSpecifier::LONG_DOUBLE:
        return FormatCheckError::INVALID_INTEGRAL_LENGTH;
    }
...
consteval FormatCheckError CheckFromat( std::string_view fmt )
{
    ...
    case 'X':
    case 'u':
        if ( !std::is_integral_v<UT> || !std::is_unsigned_v<UT> ) return FormatCheckError::EXPECTED_UNSIGNED_INTEGRAL;
        error = CheckIntegralLength( length, sizeof( UT ) );
        break;
    case 'f':
    case 'F':
    ...
    case 'a':
    case 'A':
        if ( !std::is_floating_point_v<UT> ) return FormatCheckError::EXPECTED_FLOAT_OR_DOUBLE;
        switch ( length )
        {
        case LengthSpecifier::NONE:
            break;
        case LengthSpecifier::LONG_DOUBLE:
            if ( !std::is_same_v<UT, long double> ) return FormatCheckError::EXPECTED_LONG_DOUBLE;
            break;
        default:
            return FormatCheckError::INVALID_FLOAT_LENGTH;
        }
        break;
    case 'c':
        switch ( length )
        {
        case LengthSpecifier::NONE:
            if ( !std::is_same_v<UT, char> ) return FormatCheckError::EXPECTED_CHAR_TYPE;
            break;
        case LengthSpecifier::LONG:
            if ( !std::is_same_v<UT, wchar_t> ) return FormatCheckError::EXPECTED_WIDE_CHAR_TYPE;
            break;
        default:
            return FormatCheckError::INVALID_CHAR_LENGTH;
        }
        break;
    case 's':
        switch ( length )
        {
        case LengthSpecifier::NONE:
            if ( !(std::is_pointer_v<std::decay_t<T>> && std::is_same_v<UT, char>) ) return FormatCheckError::EXPECTED_CHAR_POINTER;
            break;
        case LengthSpecifier::LONG:
            if ( !std::is_pointer_v<std::decay_t<T>> && std::is_same_v<UT, wchar_t>) return FormatCheckError::EXPECTED_WIDE_CHAR_POINTER;
            break;
        default:
            return FormatCheckError::INVALID_STRING_LENGTH;
        }
        break;
    case 'p':
        switch ( length )
        {
        case LengthSpecifier::NONE:
            if ( !std::is_pointer_v<T> ) return FormatCheckError::EXPECTED_POINTER;
            break;
        default:
            return FormatCheckError::INVALID_POINTER_LENGTH;
        }
        break;
    case 'n':
...
#define CHECK_FORMAT( fmt, ... ) \
    ...
    static_assert( error != FormatCheckError::TODO_NOT_IMPLEMENTED_YET, "TODO: haven't implemented the format check for this specifier yet (* or n)" ); \
...
#endif // #else // #if USING( USE_LOGGING )
